 ...
 * @return
 */
pipeline_framework::Queue_Factory* pipeline_framework::Queue_Factory::Instance() {
    if (!pipeline_framework::Queue_Factory::m_pInstance) {
        pipeline_framework::Queue_Factory::m_pInstance = new pipeline_framework::Queue_Factory();
    }

    return pipeline_framework::Queue_Factory::m_pInstance;
}

/**
 * ~Queue_Factory Destructor. Cleans up the queue.
 */
pipeline_framework::Queue_Factory::~Queue_Factory() {
 ...
 * @param queue_total
 */
void pipeline_framework::Queue_Factory::set_total_number_of_queues(uint8_t queue_total) {
    ...
    for (int count = 0; count < total_number_of_queues; count++) {
        ...
        str_count.append(pipeline_framework::State_Factory::convert_state_to_name(count));

        LOG_ERROR("Registering %s with one second timer.\n", str_count.c_str());

        one_second_timer_factory::Instance()->register_for_one_second_timer(
                str_count,
                pipeline_framework::Queue_Factory::Instance()->return_current_queue_count);
 ...
 * @return
 */
long pipeline_framework::Queue_Factory::return_current_queue_count(void *arg) {
...
#if 0
    for (index = 0;
            index < pipeline_framework::State_Factory::get_total_number_of_states();
            index++) {
        std::string str_count(QUEUE_CURRENT_COUNT);
        str_count.append(pipeline_framework::State_Factory::convert_state_to_name(index));
    ...
    if (match_found) {
        return_value = pipeline_framework::Queue_Factory::Instance()->count(index);
    } else {
...
uint8_t pipeline_framework::Queue_Factory::enqueue(uint8_t queue_type,
        void * message,
 ...
 */
void pipeline_framework::Queue_Factory::dequeue(uint8_t queue_type, void **ppMessage, uint32_t *pMessageLength) {
    msgq_node *pTmp = NULL;
 ...
 * @return
 */
long pipeline_framework::Queue_Factory::count(uint8_t queue_type) {
    if ((queue_type < 0) ||
            (queue_type >= total_number_of_queues)) {
        LOG_ERROR("%s:%d  Input parameters are invalid.\n", __FILE__, __LINE__);
        return -1;
 ...
 * @param location
 */
void pipeline_framework::Queue_Factory::peek_at_the_front_of_queue(uint8_t queue_type,
        void **ppMessage,
