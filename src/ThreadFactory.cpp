 ...
 */
pipeline_framework::Thread_Factory *pipeline_framework::Thread_Factory::Instance() {
    if (!pipeline_framework::Thread_Factory::p_thread_factory) {
        pipeline_framework::Thread_Factory::p_thread_factory = new pipeline_framework::Thread_Factory();
    }
    return pipeline_framework::Thread_Factory::p_thread_factory;
}

/**
 * Thread_Factory Constructor. Initializes thread_var.
 */
pipeline_framework::Thread_Factory::Thread_Factory() {
    thread_var = {};
}
...
pipeline_framework::Thread_Factory::~Thread_Factory() {
    ...
    for (int index = 0; index < total_number_of_mutex; index++) {
        Pthread_variables *pthread_var = pipeline_framework::Thread_Factory::thread_var.at(index);
        ...
        pipeline_framework::Thread_Factory::thread_var.erase(index);
    }
}
 ...
 */
void *pipeline_framework::Thread_Factory::thread_main_job(void *arg) {
    ...
    thread_name = pipeline_framework::Job_Factory::convert_state_to_name(thread_id);
 ...
 */
long pipeline_framework::Thread_Factory::return_current_thread_count(void *arg) {
 ...
 */
void pipeline_framework::Thread_Factory::Create_Thread(uint8_t index) {
    pthread_attr_t attr;
    ...
    pthread_var = pipeline_framework::Thread_Factory::thread_var[index];
    ...
    pipeline_framework::Thread_Factory::thread_var[index] = pthread_var;
    ...
    pthread_create(
            pthread_ptr,
            &attr,
            &pipeline_framework::Thread_Factory::thread_main_job,
 ...
 */
void pipeline_framework::Thread_Factory::Initialize_Thread_Factory() {
    uint8_t total_number_of_threads = 0;

    total_number_of_threads = pipeline_framework::Job_Factory::get_total_number_of_states();
 ...
 */
void pipeline_framework::Thread_Factory::Add_more_threads(uint8_t thread_identifier) {
 ...
 */
void pipeline_framework::Thread_Factory::Delete_threads(uint8_t thread_identifier) {
 ...
 * per queue id.
 */
int pipeline_framework::Thread_Factory::return_total_number_of_threads_per_queue(uint8_t thread_identifier) {
    Pthread_variables *pthread_var = NULL;
    pthread_var = pipeline_framework::Thread_Factory::thread_var[thread_identifier];
