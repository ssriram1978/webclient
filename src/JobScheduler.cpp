...
pipeline_framework::Job_Scheduler* pipeline_framework::Job_Scheduler::m_pInstance = NULL;

/**
 * Scheduler_Factory Constructor.
 */
pipeline_framework::Job_Scheduler::Job_Scheduler() {
    //Nothing
}
 ...
 * @return
 */
pipeline_framework::Job_Scheduler* pipeline_framework::Job_Scheduler::Instance() {
    if (!m_pInstance) {
        m_pInstance = new pipeline_framework::Job_Scheduler();
    }
 ...
 */
void pipeline_framework::Job_Scheduler::initialize(int argc, char **argv) {
    ...
    while (current_state <= final_state) {
        ...
        one_second_timer_factory::Instance()->register_for_one_second_timer(
                str_count,
                pipeline_framework::Job_Scheduler::Instance()->return_current_dequeue_done_count);
        ...
        one_second_timer_factory::Instance()->register_for_one_second_timer(
                str_count2,
                pipeline_framework::Job_Scheduler::Instance()->return_current_enqueue_done_count);
 ...
 */
long pipeline_framework::Job_Scheduler::return_current_dequeue_done_count(void *arg) {
    ...
    if (match_found) {
        return_value = pipeline_framework::Job_Scheduler::Instance()->dequeue_job_count[index];
        pipeline_framework::Job_Scheduler::Instance()->dequeue_job_count[index] = 0;
    }
 ...
 */
long pipeline_framework::Job_Scheduler::return_current_enqueue_done_count(void *arg) {
    std::string *p_thread_name = (std::string *) arg;
    long return_value = -1;
...
#endif
        return_value = pipeline_framework::Job_Scheduler::Instance()->enqueue_job_count[index];
        pipeline_framework::Job_Scheduler::Instance()->enqueue_job_count[index] = 0;
    }
 ...
 */
void pipeline_framework::Job_Scheduler::run() {
    ...
    pipeline_framework::Mutex_Factory::Instance()->condition_signal(
            ...
            pipeline_framework::Job_Scheduler::Instance()->Enqueue_All_Jobs_to_specified_queue,
            (void *) *ptr_to_state);
 ...
 * @param state_id
 */
void pipeline_framework::Job_Scheduler::Perform_a_Job(uint8_t state_id) {
    ...
    uint64_t job_id = (uint64_t) pipeline_framework::Mutex_Factory::Instance()->condition_wait(
            state_id,
            pipeline_framework::Job_Scheduler::Dequeue_Job,
            (void *) *ptr_to_state);
    if (job_id > 0) {
        ...
        int return_code = Job_Scheduler::Execute_Job(state_id, (uint64_t) job_id);

        if (return_code == SUCCESS) {
            Job_Scheduler::Move_Job(state_id, (uint64_t) job_id);
        } else {
            //Move all jobs to init state.
            pipeline_framework::Mutex_Factory::Instance()->condition_signal(
                    pipeline_framework::Job_Factory::get_init_state(),
                    pipeline_framework::Job_Scheduler::Instance()->move_current_job_to_init_state,
                    (void *) job_id);
        }
 ...
 * @return
 */
void* pipeline_framework::Job_Scheduler::Dequeue_Job(void *p_state) {
 ...
 * @return
 */
int pipeline_framework::Job_Scheduler::Execute_Job(
        uint8_t state_id,
        uint64_t job_id) {
    if (job_id <= 0) {
 ...
 * @param p_job
 */
void pipeline_framework::Job_Scheduler::Move_Job(
        uint8_t state_id,
        uint64_t job_id) {
    if (job_id <= 0) {
        LOG_ERROR("%s:%s:%d p_job is NULL\n", __FILE__, __FUNCTION__, __LINE__);
    ...
    pipeline_framework::Mutex_Factory::Instance()->condition_signal(
            next_state,
            pipeline_framework::Job_Scheduler::Instance()->move_Job,
            (void *) job_id);
 ...
 */
void pipeline_framework::Job_Scheduler::stop() {
 ...
 * @param p_queue_id
 */
void pipeline_framework::Job_Scheduler::Enqueue_All_Jobs_to_specified_queue(void *p_queue_id) {
    uint64_t thread_data = (uint64_t) p_queue_id;
    uint8_t queue_id = (uint8_t) thread_data;

    if (queue_id < pipeline_framework::Job_Factory::get_init_state() ||
            queue_id > pipeline_framework::Job_Factory::get_max_state()) {
    ...
    pipeline_framework::Job_Factory::Instance()->get_all_jobs(job_identifiers);
...
void pipeline_framework::Job_Scheduler::move_current_job_to_init_state(void *p_job_id) {
    ...
    pipeline_framework::Job_Factory::Instance()->set_current_job_state(p_job_id,
            pipeline_framework::Job_Factory::get_init_state());
 ...
 * @param p_job
 */
void pipeline_framework::Job_Scheduler::move_Job(void *p_job_id) {
    uint64_t job_id = (uint64_t) p_job_id;
    ...
    uint8_t current_state = pipeline_framework::Job_Factory::Instance()->get_current_job_state(p_job_id);
    uint8_t next_state = pipeline_framework::Job_Factory::get_next_state(current_state);
    uint8_t init_state = pipeline_framework::Job_Factory::get_init_state();

    if (next_state == init_state) {
        LOG_DEBUG("Reached one loop of iteration..\n");
        pipeline_framework::Job_Factory::Instance()->increment_iteration_count(p_job_id);
    }

    pipeline_framework::Job_Factory::Instance()->set_current_job_state(p_job_id, next_state);

    LOG_DEBUG("Setting next state "
            "to=%d.Enqueuing the job to the next state.\n",
            pipeline_framework::Job_Factory::Instance()->get_current_job_state(p_job_id));
