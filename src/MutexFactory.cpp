...
void* pipeline_framework::Mutex_Factory::condition_wait(uint8_t state_type,
    ...
    status = pthread_cond_timedwait(p_cond, p_mutex, &ts);
    if (status == ETIMEDOUT) {
        LOG_DEBUG("%s:%d state=%d pthread_cond_timedwait timed out\n",
                __FUNCTION__, __LINE__, state_type);
        //dequeue and process the message.
    } else {
        LOG_ERROR("%s:%d state=%d pthread_cond_timedwait failed\n",
                __FUNCTION__, __LINE__, state_type);
        return return_argument;
    }
